...
class DatebaseTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        with sqlite3.connect(cls.get_test_db_name()) as conn:
        ...
        cls.data = {'analytical_solutions_character': [tuple(i for i in range(0, 9))], 'analytical_solutions_weapon': [tuple(i for i in range(0, 7))]}
    ...
    def test_create_tables_on_database_doesnt_clear_existing_data(self):
        ...
        with sqlite3.connect(self.get_test_db_name()) as conn:
            for table in tables:
                current_table = table_data_to_hashtable(table, conn)
                self.assertEqual(current_table, self.data[table])
        ...
        with sqlite3.connect(self.get_test_db_name()) as conn:
            for i in range(0,len(tables)):
                ...
                self.assertEqual(current_table, {})
                self.assertEqual(set(tables_data[i][0]),current_table )
